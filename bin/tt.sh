...
# NEW: configurable listen/bind address (defaults to localhost)
...
# --- derive -L argument safely (IPv4/IPv6) --------------------------------
# OpenSSH expects [bind_addr]:port:host:hostport if bind_addr is IPv6 literal.
build_L_arg() {
  local bind="$1" lport="$2" rhost="$3" rport="$4"
  if [[ "$bind" == *:* && "$bind" != \[*\] ]]; then
    # looks like a raw IPv6 literal; wrap in brackets
    printf '[%s]:%s:%s:%s' "$bind" "$lport" "$rhost" "$rport"
  else
    printf '%s:%s:%s:%s' "$bind" "$lport" "$rhost" "$rport"
  fi
}

L_SPEC="$(build_L_arg "$LISTEN_ADDR" "$LOCAL_PORT" "$REMOTE_HOST" "$REMOTE_PORT")"

# Friendly warnings for wide binds
case "$LISTEN_ADDR" in
  0.0.0.0|::|*\*) warn "Binding to $LISTEN_ADDR exposes the port on all interfaces. Ensure your firewall is configured.";;
esac
...
# --- logging ---------------------------------------------------------------
info "Forwarding ${LISTEN_ADDR}:${LOCAL_PORT} → ${REMOTE_HOST}:${REMOTE_PORT}"
info "Hops (${NUM_HOPS}):"
...
if [[ -n "$JUMP_CHAIN" ]]; then
  echo "  ssh -F $TMP_SSH_CONFIG -J $JUMP_CHAIN -o ExitOnForwardFailure=yes -N -L '$L_SPEC' $FINAL_ALIAS"
else
  echo "  ssh -F $TMP_SSH_CONFIG -o ExitOnForwardFailure=yes -N -L '$L_SPEC' $FINAL_ALIAS"
fi
echo
